...
- 看门狗机制能处理卡住的任务
- 服务重启后能继续处理任务
- ✅ **[2026-01-16 已修复]** 内容更新失败时任务正确标记为 `failed`

**前端轮询和重连机制：完整 ✅**
- 有完善的轮询管理器（SSE + 轮询混合）
- 网络错误处理机制良好
- 页面刷新后能自动恢复
- ✅ **[2026-01-16 已实现]** 页面可见性监听（后台恢复时重启轮询）
- ✅ **[2026-01-16 已实现]** 网络恢复监听（`online` 事件自动重启轮询）
- ✅ **[2026-01-16 已优化]** 轮询状态完整恢复（包括 attemptCount 和 interval）

## ✅ 已完成的优化 (2026-01-16)

### 🔴 高优先级（已完成）
1. **✅ 内容更新失败处理**：AI 生成成功但 content 更新失败时，任务正确标记为 `failed` 而不是 `done`
   - 文件：`asyncGenerationQueue.js` (line 649-699)
   
2. **✅ 轮询间隔完整恢复**：页面刷新后同时恢复 attemptCount 和 interval 状态
   - 文件：`generationStatus.ts` (restorePollingState 方法)

3. **✅ 网络恢复自动重启**：添加 `online` 事件监听，网络恢复时主动重启状态监控
   - 文件：`ContentCard.tsx` (handleOnline 事件)

## 🎯 后续优化建议

### 中优先级
1. **添加批量状态查询**：列表页面有多个进行中任务时，减少网络请求
2. **添加浏览器通知**：✅ 已实现，但需要用户授权

### 低优先级
3. **服务重启任务恢复**：考虑延迟标记 processing 任务为失败，或自动重试
4. **离线状态用户提示**：当检测到离线时，显示友好提示

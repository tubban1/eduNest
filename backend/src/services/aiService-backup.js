...
const generateEducationalContent = async (knowledgePoint, learningStage, description = '') => {
  try {
    ...
    try {
      console.log('开始JSON解析，原始内容长度:', aiResponse.length);
      
      // 尝试多种JSON匹配模式，优先选择最完整的JSON
      const allMatches = [];
      
      // 1. 首先尝试直接解析整个响应（最可靠）
      try {
        const parsed = JSON.parse(aiResponse);
        console.log('✅ 直接解析整个响应成功');
        parsedData = parsed;
        return parsedData;
      } catch (e) {
      ...
      // 2. 标准JSON匹配（最完整）
      const standardMatch = aiResponse.match(/\{[\s\S]*\}/);
      if (standardMatch) {
        console.log('找到标准JSON匹配，长度:', standardMatch[0].length);
        allMatches.push(standardMatch[0]);
      }
      
      // 3. 代码块中的JSON
      const codeBlockMatch = aiResponse.match(/```json\s*(\{[\s\S]*?\})\s*```/);
      if (codeBlockMatch) {
        console.log('找到代码块JSON匹配，长度:', codeBlockMatch[1].length);
        allMatches.push(codeBlockMatch[1]);
      }
      
      // 4. 查找最大的JSON对象（更精确的正则表达式）
      const largeJsonMatch = aiResponse.match(/\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}/);
      if (largeJsonMatch) {
        console.log('找到大型JSON匹配，长度:', largeJsonMatch[0].length);
        allMatches.push(largeJsonMatch[0]);
      }
      
      console.log('总共找到匹配项数量:', allMatches.length);
      ...
      for (let i = 0; i < allMatches.length; i++) {
        ...
        try {
          // 尝试解析，验证JSON完整性
          JSON.parse(match);
          console.log(`匹配项 ${i + 1} 是有效JSON，长度: ${match.length}`);
          if (match.length > maxLength) {
            maxLength = match.length;
            bestMatch = match;
          }
        } catch (e) {
          console.log(`匹配项 ${i + 1} 不是有效JSON，跳过`);
          continue;
        }
      }
      ...
      if (bestMatch) {
        jsonMatch = [bestMatch];
        console.log('找到最佳JSON匹配，长度:', jsonMatch[0].length);
        console.log('JSON内容预览:', jsonMatch[0].substring(0, 200) + '...');
        parsedData = JSON.parse(jsonMatch[0]);
        console.log('解析后的数据字段:', Object.keys(parsedData));
      } else {
        console.error('无法找到有效的JSON结构，原始内容长度:', aiResponse.length);
        console.error('原始内容预览:', aiResponse.substring(0, 500) + '...');
        throw new Error('无法解析AI返回的JSON，请检查AI返回的格式');
      }
    } catch (parseError) {
      console.error('JSON解析错误:', parseError);
      console.error('尝试解析的内容长度:', jsonMatch ? jsonMatch[0].length : 0);
      if (jsonMatch) {
        console.error('JSON内容预览:', jsonMatch[0].substring(0, 200) + '...');
      }
...
const fixEducationalContent = async ({ html, css, js, external_links, note, content_type, language, title, description }) => {
  ...
  https://cdnjs.cloudflare.com/ajax/libs/tone/14.8.49/Tone.min.js
    ...
    let jsonMatch = null; // 声明在外部作用域
    try {
      console.log('开始修复JSON解析，原始内容长度:', aiResponse.length);
      ...
      // 1. 标准JSON匹配 - 修复正则表达式
      const standardMatch = processedResponse.match(/\{[\s\S]*\}/);
      if (standardMatch) {
        console.log('找到修复标准JSON匹配，长度:', standardMatch[0].length);
        allMatches.push(standardMatch[0]);
      }
      ...
      if (codeBlockMatch) {
        console.log('找到修复代码块JSON匹配，长度:', codeBlockMatch[1].length);
        allMatches.push(codeBlockMatch[1]);
      }
      
      // 3. 查找所有可能的JSON对象
      const allJsonMatches = processedResponse.match(/\{[\s\S]*?\}/g);
      if (allJsonMatches) {
        console.log('找到修复多个JSON匹配，数量:', allJsonMatches.length);
        allMatches.push(...allJsonMatches);
      }
      
      // 4. 查找最后一个可能的JSON结构
      const lastPossibleJson = processedResponse.match(/\{[^{}]*"[^{}]*"[^{}]*\}/);
      if (lastPossibleJson) {
        console.log('找到修复简化JSON匹配，长度:', lastPossibleJson[0].length);
        allMatches.push(lastPossibleJson[0]);
      }
      
      console.log('修复总共找到匹配项数量:', allMatches.length);
      ...
      for (let i = 0; i < allMatches.length; i++) {
        ...
        try {
          // 尝试解析，验证JSON完整性
          JSON.parse(match);
          console.log(`修复匹配项 ${i + 1} 是有效JSON，长度: ${match.length}`);
          if (match.length > maxLength) {
            maxLength = match.length;
            bestMatch = match;
          }
        } catch (e) {
          console.log(`修复匹配项 ${i + 1} 不是有效JSON，跳过`);
          continue;
        }
      }
      ...
      if (bestMatch) {
        jsonMatch = [bestMatch];
        console.log('找到最佳修复JSON匹配，长度:', jsonMatch[0].length);
        console.log('修复JSON内容预览:', jsonMatch[0].substring(0, 200) + '...');
        parsed = JSON.parse(jsonMatch[0]);
        console.log('修复解析后的数据字段:', Object.keys(parsed));
      } else {
        console.error('无法找到有效的修复JSON结构，原始内容长度:', aiResponse.length);
        console.error('原始内容预览:', aiResponse.substring(0, 500) + '...');
      ...
      console.error('尝试解析的内容长度:', jsonMatch ? jsonMatch[0].length : 0);
      if (jsonMatch) {
        console.error('修复JSON内容预览:', jsonMatch[0].substring(0, 200) + '...');
      }
